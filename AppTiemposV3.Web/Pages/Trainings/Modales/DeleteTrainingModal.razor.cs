...
public partial class DeleteTrainingModal : ComponentBase
{
    #region Variables
    ...
    #region Funciones
    private async Task DeleteTrainingAc(Guid id)
    {
        try
        {
            await JS!.InvokeVoidAsync("modalHelpers.clickElement", showModalRef);
            StateHasChanged();
            GeneralResponse? response = await TrainingService.DeleteTraining(id);
